RSpec.describe 'trade event', type: :feature, js: true do
  context 'when viewing DL trade event page' do
    it 'renders fields' do
      ...
      expect(find_by_id('field_name').find('h1').text).to eq('2016 fall JJL Education Fair in Wuhan')
  ...
  context 'when viewing SBA trade event page' do
    it 'renders fields' do
      ...
      expect(find_by_id('field_name').find('h1').text).to eq('Vietnam Natural Gas Sector Business Briefing')
      expect(find_by_id('field_url'))[:href]).to eq('https://www.export.gov/tradeevent?id=e9762d0d3239624747a1b1e06497816f6e085268')
      ...
      expect(find_by_id('field_registration_url'))[:href]).to eq('https://www.regonline.com/Register/Checkin.aspx?EventID=1825401')
